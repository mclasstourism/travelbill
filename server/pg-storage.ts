...
export class PgStorage implements IStorage {
  ...
  async getCashReceipts(): Promise<CashReceipt[]> {
    ...
    return results.map(r => ({
      ...
      airlinesFlightNo: r.airlinesFlightNo || "",
      tktNo: r.tktNo || "",
      departureTime: r.departureTime || "",
      arrivalTime: r.arrivalTime || "",
      basicFare: r.basicFare || 0,
  ...
  async getCashReceipt(id: string): Promise<CashReceipt | undefined> {
    ...
    return {
      ...
      airlinesFlightNo: result.airlinesFlightNo || "",
      tktNo: result.tktNo || "",
      departureTime: result.departureTime || "",
      arrivalTime: result.arrivalTime || "",
      basicFare: result.basicFare || 0,
  ...
  async createCashReceipt(receipt: InsertCashReceipt): Promise<CashReceipt> {
    ...
    const [result] = await db.insert(schema.cashReceiptsTable).values({
      ...
      basicFare: receipt.basicFare || 0,
      amount: receipt.amount,
      totalAmount: receipt.totalAmount || 0,
      receivedAmount: receipt.receivedAmount || 0,
      dueAmount: receipt.dueAmount || 0,
      items: receipt.items || [],
