...
export default function VendorTransactionReportPage() {
  ...
  const getVendorName = (vendorId: string) => {
    return vendorMap.get(vendorId)?.name || "Unknown";
  };
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return filtered;
  }, [transactions, searchQuery, vendorFilter, dateFilter]);
  ...
  const handleExportExcel = () => {
    ...
    const headers = ["Date", "Vendor Name", "Description", "Category", "Type", "Amount", "Balance After"];
    ...
    link.download = `vendor_transaction_report_${format(new Date(), "yyyyMMdd")}.csv`;
    link.click();
  };
  ...
  const handlePrintPdf = async () => {
    ...
    const printContent = `
      ...
      <html>
        <head>
          <title>Vendor Transaction Report</title>
          <style>
        ...
        <body>
          ...
          <p class="report-title">VENDOR TRANSACTION REPORT</p>
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            View all vendor credit and deposit transactions with filters and export
          </p>
        </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">Adjust filters or add vendor transactions</p>
            </div>
          ) : (
